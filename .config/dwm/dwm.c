...
buttonpress(XEvent *e)
{
	...
	if (ev->window == selmon->barwin) {
		...
		// calculate total width of all visible tag icons
		for (i = 0; i < LENGTH(tags); i++)
			tagwidth += TEXTW(selmon->tag_icons[i]);

		x += tagwidth;

		if (ev->x >= x && ev->x < x + TEXTW(selmon->ltsymbol)) {
			click = ClkLtSymbol;
			...
			// Click was within the tag area, do nothing
			return;
		}
...
drawbar(Monitor *m)
{
	...
	if ((w = m->ww - tw - x) > bh) {
		if (m->sel) {
			...
			/* Extract clean title after last dash */
			char cleanname[256];
			strncpy(cleanname, m->sel->name, sizeof(cleanname) - 1);
			cleanname[sizeof(cleanname) - 1] = '\0';

			char *dash = strrchr(cleanname, '-');
			char *final = (dash && *(dash + 1)) ? dash + 1 : cleanname;

			while (*final == ' ')
				final++;

			tlpad = MAX((m->ww - ((int)TEXTW(final) - lrpad)) / 2 - x, lrpad / 2);
			drw_text(drw, x, 0, w, bh, tlpad, final, 0);
...
static void scratchpad_show_client (Client * c)
{
	...
	c->isfloating = 0; // <-- This line forces it to tile
	focus(c);
	arrange(selmon);
